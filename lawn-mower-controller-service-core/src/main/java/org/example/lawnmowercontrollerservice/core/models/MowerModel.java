...
public class MowerModel {
    ...
    public String executeInstructions(AxisDTO upperRightAxis) {
        for (char c : this.instructions.toCharArray()) {
            switch (c) {
                case 'L' -> this.coordinatesDTO.setDirection(this.coordinatesDTO.getDirection().previous());
                case 'R' -> this.coordinatesDTO.setDirection(this.coordinatesDTO.getDirection().next());
